public class Board
 {
    ...
    public void initializeBoard() {
        ...
        // the privileged pieces
        for (int u = 0; u < 8; u++) {
            board[u][6] = new Pawn(true);
        }
    ...
    public boolean testCastling(int oldX, int oldY, int newX, int newY) {
        ...
        if (board[oldX][oldY] != null) {
            ...
            if (pieceName.equalsIgnoreCase("bk") && board[oldX][oldY].firstMove == true) {
                //is the king black and hasnt been moved
                ...
                if (deltax == -2) {
                    ...
                    if (board[0][0] != null) {
                        ...
                        if (board[0][0].drawPiece().equalsIgnoreCase("br")) {
                            //check if the rook is in its original place
    ...
    public boolean detectEnPassant(int oldX, int oldY, int newX, int newY) {
        //call this after the move had been performed for this turn
        ...
        if (!(board[newX][newY].drawPiece().equalsIgnoreCase("wp") || board[newX][newY].drawPiece().equalsIgnoreCase("bp"))) {
            // white pawn or black is in the new location
            return false;
        }
        if (Math.abs(newY - oldY) != 2) {
            //white pawn had just moved forward by 2
            return false;
        }
    ...
    */
    public boolean detectCheck(boolean whiteturn) {
        ...
        if (whiteturn == true) {
            //if whiteturn is false, check if the white is in check
            for (int y = 0; y < 8; y++) {
                for (int x = 0; x < 8; x++) {
                    if (board[x][y] != null) {
        ...
        if (whiteturn == false) {
            //if whiteturn is true, check if the black is in check
            for (int y = 0; y < 8; y++) {
                for (int x = 0; x < 8; x++) {
                    if (board[x][y] != null) {
